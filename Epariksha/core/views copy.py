...
def take_exam(request, exam_id):
    ...
    return render(request, 'core/take_exam.html', {'exam': exam})
...
def view_submissions(request, exam_id):
    ...
    for submission in submissions:
        ...
        if submission.bert_time is not None:
            # submission.bert_time_formatted = f"{submission.bert_time:.3f}s"
...
def evaluate_submission(request, submission_id):
    ...
    for answer in answers:
        ...
        line_count = student_text_combined.count('\n')
        ...
        if case_type in ["both", "descriptive"]:  
            ...
            try:
                summary_list = summarizer(student_desc, max_length=max_len, min_length=min_len, do_sample=False)
        ...
        if student_code:
            try:
                with tempfile.NamedTemporaryFile(suffix='.c', delete=True) as source_file:
                    ...
                    output_file = os.path.join(tempfile.gettempdir(), 'a.exe' if platform.system() == 'Windows' else 'a.out')

                    compile_proc = subprocess.run(
                        ['gcc', source_file.name, '-o', output_file],
                        capture_output=True,
                        text=True,
                        timeout=5
                    )
    ...
    if request.method == 'POST':
        ...
        for item in answers_data:
            ...
            ans.teacher_feedback = request.POST.get(f'feedback_{ans.id}', '')
